...
/* =========================================================
   ...
   Handles optional cover photo upload.
   ========================================================= */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $cover_photo = ''; // will stay empty if no file uploaded
    ...
    if ($trip_name === '' || $start_date === '' || $end_date === '') {
        ...
        /* -----------------------------------------------
           ...
           We only process it if a file was actually chosen.
           ----------------------------------------------- */
        ...
        // Insert the new trip into the database using a prepared statement
        $stmt = $conn->prepare("INSERT INTO trips (user_id, trip_name, start_date, end_date, description, cover_photo)
                                 VALUES (?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("isssss", $user_id, $trip_name, $start_date, $end_date, $description, $cover_photo);

        if ($stmt->execute()) {
            $new_trip_id = $stmt->insert_id;
            $stmt->close();
            // Redirect straight into the itinerary builder for this new trip
            header('Location: itinerary-builder.php?trip_id=' . $new_trip_id);
            exit;
        } else {
...
<div class="container py-5">
    <div class="row justify-content-center">
        <div class="col-lg-7">
            ...
            <div class="card">
                <div class="card-body">
                    <!-- enctype is required because we allow a file (cover photo) upload -->
